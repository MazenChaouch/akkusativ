...
type Mode = "generator" | "genus" | "review" | "vocab";
...
const CAT_NAME: Record<Category, string> = {
  def: "Bestimmte Artikel",
  indef: "Unbestimmte Artikel",
...
const MODES: { id: Mode; label: string; icon: typeof Dices }[] = [
  ...
  { id: "genus", label: "Genus-Trainer", icon: GraduationCap },
...
export const TrainingPart = ({ onGoRules }: { onGoRules: () => void; onPdf: () => void }) => {
  ...
  const check = (ex: GenExercise, isReview: boolean) => {
    ...
    setPersist((s) => {
      ...
      // score only the first attempt of each exercise
      if (prevAttempts === 0) {
        ...
        // remember the mistake so it can be retrained later
        if (!allOk && !isReview) {
      ...
      <div className="max-w-4xl mx-auto px-5 md:px-8">
        ...
        <Reveal>
          ...
          <p className="mt-5 text-lg text-ink-soft max-w-xl leading-relaxed">
            Vier Trainingsarten, ein Ziel: Der Akkusativ soll dir in Fleisch und Blut übergehen.
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="gen" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                ...
                <Reveal>
                  <div className="mb-6 flex items-start gap-3.5 rounded-2xl border-2 border-accent/40 bg-accent/[0.05] px-5 py-4">
                    ...
                    <p className="text-[14.5px] leading-relaxed text-ink/85">
                      ...
                      {weakest.c === "pron" && " Wiederhole Kapitel 4 — besonders wir→uns und ihr→euch."}
                      {weakest.c === "poss" && " Wiederhole Kapitel 5: Possessive folgen dem ein-Pattern."}
                      {weakest.c === "def" && " Wiederhole Kapitel 2: nur der → den."}
                      {weakest.c === "indef" && " Wiederhole Kapitel 3: nur ein → einen."}
                      {weakest.c === "mixed" && " Wiederhole Kapitel 7: erst Subjekt/Objekt klären, dann Genus."}
                    </p>
                  </div>
                </Reveal>
              )}
              ...
              <AnimatePresence>
                {allSolved && (
                  ...
                  >
                    <span aria-hidden className="absolute -right-8 -top-14 font-display font-black text-[13rem] leading-none text-white/[0.05] select-none">n</span>
                    <motion.div
                ...
                <div className="mt-14 rounded-3xl border-2 border-ink bg-white/60 p-6 md:p-8">
                  <div className="flex items-center justify-between gap-4 flex-wrap mb-6">
                    <div>
                      ...
                      <h3 className="font-display text-2xl md:text-3xl font-[700]">Wo sitzt das kleine n schon?</h3>
                    </div>
                    <button
                  ...
                  <div className="grid md:grid-cols-2 gap-x-10 gap-y-5">
                    <div className="space-y-4">
                      ...
                      {(["def", "indef", "poss", "pron", "mixed"] as Category[]).map((c) => (
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-[12px] text-ink-soft leading-relaxed pt-1">
                        Prozent = beim <b>ersten Versuch</b> richtig. Maskulin ist erfahrungsgemäß am schwersten —
                        dort lebt das -n.
                      </p>
                    </div>
                  </div>
          ...
          {/* ================= GENUS ================= */}
          {mode === "genus" && (
            <motion.div key="genus" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="mt-8">
              <GenusTrainer onSpeak={say} audio={supported && enabled} />
            </motion.div>
          )}
              ...
              <Reveal>
                <div className="rounded-2xl border border-line bg-white/55 p-5 mb-6">
                  <p className="text-[15px] leading-relaxed text-ink/80">
                    Alle Nomen dieses Buches mit Artikel, Plural und Akkusativ-Form. Genau diese Wörter
                ...
                {vocabList.slice(0, vocabShown).map((n) => (
                  <div
                    ...
                    className={`rounded-2xl border-2 p-4 transition-colors ${
                      n.g === "m" ? "border-accent/45 bg-accent/[0.04]" : "border-line bg-white/55"
                    }`}
                  >
              ...
              <Reveal>
                <div className="mt-10 rounded-2xl border border-line bg-white/55 p-5 md:p-6">
                  <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent mb-4">
                    Die Verben — sie alle verlangen den Akkusativ
                  </div>
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-2.5">
                    {(Object.keys(VERBS) as (keyof typeof VERBS)[]).map((v) => (
                      <div key={v} className="flex items-baseline justify-between gap-2 border-b border-line/60 pb-1.5">
                        <span className="font-display text-[16px] font-[650]">{VERBS[v].inf}</span>
                        <span className="text-[12px] text-ink-soft italic">{VERBS[v].en}</span>
                      </div>
                    ))}
                  </div>
                  <p className="text-[12.5px] text-ink-soft mt-4 leading-relaxed">
                    Diese Verben nennt man <b>transitive Verben</b>: Sie brauchen ein direktes Objekt im Akkusativ.
                    Genau deshalb tauchen sie in jedem Beispiel dieses Buches auf.
                  </p>
                </div>
              </Reveal>
            </motion.div>
