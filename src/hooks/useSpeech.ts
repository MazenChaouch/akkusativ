...
export function useSpeech() {
  ...
  const [enabled, setEnabled] = useState(() => {
    try { return localStorage.getItem("akk-audio") !== "off"; } catch { return true; }
  });
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      if (de.length) {
        // prefer a natural / local voice, then plain de-DE
        voiceRef.current =
  ...
  useEffect(() => {
    try { localStorage.setItem("akk-audio", enabled ? "on" : "off"); } catch { /* ignore */ }
  }, [enabled]);
